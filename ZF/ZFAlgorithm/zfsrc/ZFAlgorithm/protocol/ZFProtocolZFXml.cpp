...
static zfindex _ZFP_ZFXml_decimalCharCheck(
        ZF_IN const zfchar *head
...
static void _ZFP_ZFXml_decimalCharEscape(
        ZF_IN_OUT const ZFOutput &dst
    ...
    if(zfsToIntT(*(zft_zfuint16 *)s, p, encodedCharLen, 10)) {
        zfstring t;
        zfstringToUTF8(t, v_ZFStringEncoding::e_UTF16, s);
...
static zfindex _ZFP_ZFXml_heximalCharCheck(
        ZF_IN const zfchar *head
        , ZF_IN const zfchar *end
        ) { // 0; ~ FFFF;
    const zfchar *p = head;
...
static void _ZFP_ZFXml_heximalCharEscape(
        ZF_IN_OUT const ZFOutput &dst
        , ZF_IN_OUT const zfchar *&p
        , ZF_IN zfindex encodedCharLen
        ) {
...
void ZFPROTOCOL_INTERFACE_CLASS(ZFXml)::xmlEscapeCharEncode(
...
void ZFPROTOCOL_INTERFACE_CLASS(ZFXml)::xmlEscapeCharDecode(
        ZF_OUT const ZFOutput &dst
        , ZF_IN const zfchar *src
        , ZF_IN_OPT zfindex count /* = zfindexMax() */
        ) {
    ...
    while(p < pEnd) {
        if(*p == '&') {
            ...
            // &#0; ~ &#65536;
                ...
                p += 2;
                _ZFP_ZFXml_decimalCharEscape(dst, p, encodedCharLen);
                pLeft = p;
            }
            // &#x0; ~ &#xFFFF;
                ...
                p += 3;
                _ZFP_ZFXml_heximalCharEscape(dst, p, encodedCharLen);
                pLeft = p;
            }
