...
public:
    ...
    ZFJsonType type;
    union {
        zfstring *value; // for v_ZFJsonType::e_Value
        AttrMap *attrMap; // for v_ZFJsonType::e_Object
        ChildList *childList; // for v_ZFJsonType::e_Array
    } d;
...
ZFJsonType ZFJson::type(void) const {
    return (d ? d->type : v_ZFJsonType::e_Null);
}
...
ZFJson &ZFJson::merge(ZF_IN const ZFJson &other) {
    ...
    switch(this->type()) {
        case v_ZFJsonType::e_Null:
            if(other.type() == v_ZFJsonType::e_Object || other.type() == v_ZFJsonType::e_Array) {
        ...
        case v_ZFJsonType::e_Value:
            break;
        case v_ZFJsonType::e_Object:
            if(other.type() == v_ZFJsonType::e_Object) {
                for(zfiter it = other.attrIter(); it; ++it) {
                    ...
                    if(exist.type() == v_ZFJsonType::e_Object && attach.type() == v_ZFJsonType::e_Object) {
                        exist.merge(attach);
                    }
                    else {
            ...
            break;
        case v_ZFJsonType::e_Array:
            if(other.type() == v_ZFJsonType::e_Array) {
                this->childRemoveAll();
...
const zfstring &ZFJson::value(void) const {
    return d && d->type == v_ZFJsonType::e_Value && d->d.value ? *(d->d.value) : zfstring::Empty();
}

// ============================================================
// for object type
zfindex ZFJson::attrCount(void) const {
    return d && d->type == v_ZFJsonType::e_Object && d->d.attrMap ? (zfindex)d->d.attrMap->size() : 0;
}
...
ZFJson &ZFJson::attr(
    ...
    if(value == zfnull) {
        ...
        ZFJson tmp(v_ZFJsonType::e_Value);
        tmp.value(value);
        this->attr(key, tmp);
    }
    return *this;
...
ZFJson ZFJson::attr(ZF_IN const zfstring &key) const {
    if(d && d->type == v_ZFJsonType::e_Object && d->d.attrMap) {
...
zfstring ZFJson::attrValue(ZF_IN const zfstring &key) const {
    if(d && d->type == v_ZFJsonType::e_Object && d->d.attrMap) {
        _ZFP_ZFJsonPrivate::AttrMap::iterator it = d->d.attrMap->find(key);
        if(it != d->d.attrMap->end() && it->second.type() == v_ZFJsonType::e_Value) {
...
zfindex ZFJson::childCount(void) const {
    if(d && d->type == v_ZFJsonType::e_Array && d->d.childList) {
...
ZFJson ZFJson::childAt(ZF_IN zfindex index) const {
    if(d && d->type == v_ZFJsonType::e_Array && d->d.childList && index < (zfindex)d->d.childList->size()) {
...
ZFJson &ZFJson::child(
    ...
    return this->child(ZFJson(v_ZFJsonType::e_Value).value(value), index);
}
ZFJson &ZFJson::child(
        ZF_IN const ZFJson &item
        , ZF_IN_OPT zfindex index /* = zfindexMax() */
...
// ============================================================
ZFMETHOD_FUNC_DEFINE_0(ZFJson, ZFJsonObject) {return ZFJson(v_ZFJsonType::e_Object);}
ZFMETHOD_FUNC_DEFINE_0(ZFJson, ZFJsonArray) {return ZFJson(v_ZFJsonType::e_Array);}
...
ZFMETHOD_FUNC_DEFINE_3(zfbool, ZFJsonToOutput
    ...
    if(output && item) {
        ...
        if(item.type() == v_ZFJsonType::e_Value) {
            output << item.value();
        }
        else {
