...
protected:
    ...
    : pimplOwner(zfnull)
    , state(v_ZFUIScrollViewState::e_Idle)
...
public:
    ...
    void processDrag(
        ...
        if(this->state != v_ZFUIScrollViewState::e_Dragging) {
            return;
        }
...
private:
    ...
    void notifyScrollOnDragBegin(void) {
        if(this->state != v_ZFUIScrollViewState::e_Dragging) {
            ...
            this->state = v_ZFUIScrollViewState::e_Dragging;
    ...
    void notifyScrollOnDragEnd(void) {
        if(this->state == v_ZFUIScrollViewState::e_Dragging) {
            ...
            #endif
            this->state = v_ZFUIScrollViewState::e_Idle;
...
void ZFUIScrollView::objectOnInit(void) {
    ...
    d = zfobjAlloc(_ZFP_ZFUIScrollViewPrivate);
    d->pimplOwner = this;
    d->xScroll = zfobjRetain(this->scrollerClass()->newInstance());
    d->yScroll = zfobjRetain(this->scrollerClass()->newInstance());
...
ZFMETHOD_DEFINE_1(ZFUIScrollView, void, scrollThumbHorizontalClass
    ...
    if(cls != zfnull) {
        d->xScrollThumb = zfobjRetain(cls->newInstance());
        if(d->xScrollThumb != zfnull) {
...
ZFMETHOD_DEFINE_1(ZFUIScrollView, void, scrollThumbVerticalClass
    ...
    if(cls != zfnull) {
        d->yScrollThumb = zfobjRetain(cls->newInstance());
        if(d->yScrollThumb != zfnull) {
...
ZFMETHOD_DEFINE_1(ZFUIScrollView, void, scrollContentFrameAnimated
    ...
    if(d->state != v_ZFUIScrollViewState::e_Dragging) {
