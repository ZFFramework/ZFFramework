...
function(zfprojConfigBefore_ZF_impl projName ZF_SRC_FILES)
    ...
    if(NOT EXISTS ${FONT_TMP_FILE})
        if(WIN32)
            execute_process(COMMAND "${ZF_ROOT_PATH}/tools/common/file_download.bat" "${FONT_URL}" "${FONT_TMP_FILE}")
            execute_process(COMMAND "${ZF_ROOT_PATH}/tools/common/unzip.bat" "${FONT_TMP_FILE}" "${FONT_TMP_PATH}")
            execute_process(COMMAND "${ZF_ROOT_PATH}/tools/common/copy_check.bat" "${FONT_TMP_PATH}" "${PROJECT_BINARY_DIR}/zfres/ZF_impl/sys_SDL/font")
        else()
            execute_process(COMMAND sh "${ZF_ROOT_PATH}/tools/common/file_download.sh" "${FONT_URL}" "${FONT_TMP_FILE}")
            execute_process(COMMAND sh "${ZF_ROOT_PATH}/tools/common/unzip.sh" "${FONT_TMP_FILE}" "${FONT_TMP_PATH}")
            execute_process(COMMAND sh "${ZF_ROOT_PATH}/tools/common/copy_check.sh" "${FONT_TMP_PATH}" "${PROJECT_BINARY_DIR}/zfres/ZF_impl/sys_SDL/font")
        endif()
    endif()
