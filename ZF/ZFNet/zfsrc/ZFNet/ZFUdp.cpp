...
zfbool ZFUdpAddr::remoteInfo(ZF_OUT zfstring &remoteAddr,
...
ZFMETHOD_USER_REGISTER_FOR_WRAPPER_FUNC_2(v_ZFUdpAddr, zfbool, remoteInfo, ZFMP_OUT(zfstring &, remoteAddr), ZFMP_OUT(zfuint &, remotePort))
...
ZFOUTPUT_TYPE_DEFINE(ZFUdpAddr, {
        ...
        {
            output << zfstringWithFormat("udp:%s:%u", remoteAddr.cString(), remotePort);
        }
        else
        {
            output << "udp:null";
                  ...
                  ZFMP_IN_OPT(zfindex, size, zfindexMax()))
{
    if(size == zfindexMax())
    {
        size = (data != zfnull ? zfslen(data) : 0);
    }
    return this->send(hostAddr, (const void *)data, size);
}
