...
void _ZFP_ZFCoreCriticalError(ZF_IN const ZFCallerInfo &callerInfo, ZF_IN const zfchar *errorHint) {
    const ZFCoreArray<ZFCoreCriticalErrorCallback> &criticalErrorCallbacks = _ZFP_ZFCoreCriticalErrorCallbacks;
    if(!criticalErrorCallbacks.isEmpty()) {
        zfstring errorHintHolder = errorHint;
        for(zfindex i = 0; i < criticalErrorCallbacks.count(); ++i) {
            criticalErrorCallbacks[i](callerInfo, errorHintHolder);
        }
    }
