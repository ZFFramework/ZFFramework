...
protected:
    ...
    typedef ZFCoreOrderMap<T_Key, T_Value, T_Hash, T_Equal> zfself;
public:
    /**
     ...
     */
    ZFCoreOrderMap(ZF_IN const zfself &ref) : d(ref.d) {
        if(d) {
            ++(d->refCount);
        }
    }
     ...
     * @brief retain the ref, to copy, use #copyFrom
     */
    ZFCoreOrderMap &operator = (ZF_IN const zfself &ref) {
...
ZFOUTPUT_TYPE_TEMPLATE(ZFM_EXPAND(typename T_Key, typename T_Value, typename T_Hash, typename T_Equal), ZFM_EXPAND(ZFCoreOrderMap<T_Key, T_Value, T_Hash, T_Equal>), {v.objectInfoT(s);})
