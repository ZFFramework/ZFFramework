...
// ============================================================
static void _ZFP_ZFIOTreePrint(
    ...
    if(ioImpl->ioFindFirst(fd, pathData)) {
        do {
            ...
            if(fd.isDir()) {
                ...
                ioImpl->ioToChild(pathDataChild, pathData, fd.name());
                if(pathDataChild) {
                    _ZFP_ZFIOTreePrint(pathDataChild, outputCallback, headToken, indentToken, indentLevel + 1, ioImpl);
                }
            }
            else {
                outputCallback << fd.name();
...
ZFMETHOD_FUNC_DEFINE_4(void, ZFIOTreePrint
    ...
    if(ioImpl != zfnull) {
        _ZFP_ZFIOTreePrint(pathInfo.pathData(), outputCallback, headToken, indentToken, 0, ioImpl);
    }
}
...
static zfbool _ZFP_ZFIOForEach(
    ...
    if(ioImpl->ioFindFirst(fd->zfv, pathInfo->zfv.pathData())) {
        do {
            zfstring childPathData;
            if(!ioImpl->ioToChild(childPathData, pathInfo->zfv.pathData(), fd->zfv.name())
                    || !childPathData
                    ) {
                return zffalse;
            }
            ...
            if(isRecursive && fd->zfv.isDir()) {
                _ZFP_ZFIOForEach(ioImpl, fileCallback, pathInfo, isRecursive, forEachFile, forEachDir);
            }

            zfstring parentPathData;
            if(!ioImpl->ioToParent(parentPathData, pathInfo->zfv.pathData())
                    || !parentPathData
                    ) {
                return zffalse;
            }
...
ZFMETHOD_FUNC_DEFINE_3(zfbool, ZFIOForEach
    ...
    return _ZFP_ZFIOForEach(ioImpl, fileCallback, pathInfoHolder, isRecursive, zftrue, zftrue);
}
ZFMETHOD_FUNC_DEFINE_3(zfbool, ZFIOForEachFile
    ...
    zfobj<v_ZFPathInfo> pathInfoHolder;
    pathInfoHolder->zfv = pathInfo;
    return _ZFP_ZFIOForEach(ioImpl, fileCallback, pathInfoHolder, isRecursive, zftrue, zffalse);
}
ZFMETHOD_FUNC_DEFINE_3(zfbool, ZFIOForEachDir
    ...
    zfobj<v_ZFPathInfo> pathInfoHolder;
    pathInfoHolder->zfv = pathInfo;
    return _ZFP_ZFIOForEach(ioImpl, fileCallback, pathInfoHolder, isRecursive, zffalse, zftrue);
}
...
static zfbool _ZFP_ZFIOCopy_copyFile(
    ...
    if(isForce) {
        dstImpl->ioRemove(dstPath.pathData(), zffalse, isForce);
    }
    zfautoT<ZFIOToken> srcFd = srcImpl->ioOpen(srcPath.pathData(), v_ZFIOOpenOption::e_Read, zffalse);
    if(srcFd == zfnull) {
        return zffalse;
    }
    zfautoT<ZFIOToken> dstFd = dstImpl->ioOpen(dstPath.pathData(), v_ZFIOOpenOption::e_Write, zftrue);
    if(dstFd == zfnull) {
        return zffalse;
    }
    ...
    zfchar buf[1024];
    zfindex size = 0;
    while((size = srcFd->ioRead(buf, bufSize)) > 0) {
        if(dstFd->ioWrite(buf, size) < size) {
            return zffalse;
        }
    }
    return zftrue;
...
static zfbool _ZFP_ZFIOCopy_copyDir(
    ...
    while(!stacksDirSrc.isEmpty()) {
        ...
        if(!dstImpl->ioPathCreate(dstDir.pathData(), zftrue)) {
            return zffalse;
        }

        ZFIOFindData fd;
        if(srcImpl->ioFindFirst(fd, srcDir.pathData())) {
            ZFPathInfo srcTmp(srcDir.pathType(), zfnull);
            ZFPathInfo dstTmp(dstDir.pathType(), zfnull);
            do {
                zfstring srcPathData;
                srcImpl->ioToChild(srcPathData, srcDir.pathData(), fd.name());
                srcTmp.pathData(srcPathData);

                zfstring dstPathData;
                dstImpl->ioToChild(dstPathData, dstDir.pathData(), fd.name());
                dstTmp.pathData(dstPathData);

                if(srcImpl->ioIsDir(srcTmp.pathData())) {
...
ZFMETHOD_FUNC_DEFINE_4(zfbool, ZFIOCopy
    ...
    zfautoT<ZFIOImpl> srcImpl = ZFIOImplForPathType(srcPath.pathType());
    zfautoT<ZFIOImpl> dstImpl = ZFIOImplForPathType(dstPath.pathType());
    if(srcImpl == zfnull) {
        return zffalse;
    }
    if(dstImpl == zfnull) {
    ...
    if(!srcImpl->ioIsExist(srcPath.pathData())) {
        return zffalse;
    }
    zfbool srcIsDir = srcImpl->ioIsDir(srcPath.pathData());
    if(srcIsDir && !isRecursive) {
        return zffalse;
    }
    zfbool dstExist = dstImpl->ioIsExist(dstPath.pathData());
    zfbool dstIsDir = dstImpl->ioIsDir(dstPath.pathData());
...

    {
        ...
        if(srcIsDir) {
            ...
            dstImpl->ioToParent(dstPathDataParent, dstPath.pathData());
        }
        if(!dstImpl->ioPathCreate(dstPathDataParent, zftrue)) {
            return zffalse;
        }
    }
